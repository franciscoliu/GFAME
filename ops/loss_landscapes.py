...
def get_gnn_loss_landscape(model, n_ff, g_dataset, transform=None,
                       bases=None, kws=None,
                       cutoffs=(0.0, 0.9), bins=np.linspace(0.0, 1.0, 11), verbose=False, period=10, gpu=True,
                       x_min=-1.0, x_max=1.0, n_x=11, y_min=-1.0, y_max=1.0, n_y=11, train_mask=None, labels=None):
    model = model.cuda() if gpu else model.cpu()
    model = copy.deepcopy(model)
    ws0 = copy.deepcopy(model.state_dict())
    kws = [] if kws is None else kws
    ...
    for ratio in ratio_grid.reshape([-1, 2]):
        ...
        print("Grid: ", ratio, end=", ")
        *metrics, cal_diag = tests.gnn_test(model, n_ff, g_dataset, transform=transform,
                                        cutoffs=cutoffs, bins=bins, verbose=verbose, period=period, gpu=gpu, train_mask=train_mask, labels=labels)
        l1, l2 = norm.l1(model, gpu).item(), norm.l2(model, gpu).item()
        metrics_grid[tuple(ratio)] = (l1, l2, *metrics)

    return metrics_grid

def get_gnn_F_loss_landscape(model, n_ff, g_dataset='cora', transform=None,
                       bases=None, kws=None,
                       cutoffs=(0.0, 0.9), bins=np.linspace(0.0, 1.0, 11), verbose=False, period=10, gpu=True,
                       x_min=-1.0, x_max=1.0, n_x=11, y_min=-1.0, y_max=1.0, n_y=11, labels=None):
    model = model.cuda() if gpu else model.cpu()
    model = copy.deepcopy(model)
    ws0 = copy.deepcopy(model.state_dict())
    kws = [] if kws is None else kws
    ...
    for ratio in ratio_grid.reshape([-1, 2]):
        ...
        print("Grid: ", ratio, end=", ")
        *metrics, cal_diag = tests.gnn_f_test(model, n_ff, g_dataset, transform=transform,
                                        cutoffs=cutoffs, bins=bins, verbose=verbose, period=period, gpu=gpu, labels=labels)
        l1, l2 = norm.l1(model, gpu).item(), norm.l2(model, gpu).item()
        metrics_grid[tuple(ratio)] = (l1, l2, *metrics)

    return metrics_grid

def get_fairness_loss(model, n_ff, train_mask, dataset,
                       bases=None, kws=None,
                       cutoffs=(0.0, 0.9), bins=np.linspace(0.0, 1.0, 11), verbose=False, gpu=True,
                       x_min=-1.0, x_max=1.0, n_x=11, y_min=-1.0, y_max=1.0, n_y=11, labels=None, logits=None,
                       output_link_logits=None,output_tr_labels=None):
    model = model.cuda() if gpu else model.cpu()
    model = copy.deepcopy(model)
    ws0 = copy.deepcopy(model.state_dict())
    kws = [] if kws is None else kws
    ...
    for ratio in ratio_grid.reshape([-1, 2]):
        ...
        print("Grid: ", ratio, end=", ")
        # model, n_ff, dataset, train_mask, labels,
        *metrics, cal_diag = tests.test_fairness(model=model, n_ff=n_ff, dataset=dataset, train_mask=train_mask,
                                        cutoffs=cutoffs, bins=bins, verbose=verbose, gpu=gpu, labels=labels, logits=logits, output_link_logits= output_link_logits,
                                        output_tr_labels=output_tr_labels)
        l1, l2 = norm.l1(model, gpu).item(), norm.l2(model, gpu).item()
        metrics_grid[tuple(ratio)] = (l1, l2, *metrics)

    return metrics_grid

def get_rgnn_loss_landscape(model, n_ff, train_mask, dataset,
                       bases=None, kws=None,
                       cutoffs=(0.0, 0.9), bins=np.linspace(0.0, 1.0, 11), verbose=False, gpu=True,
                       x_min=-1.0, x_max=1.0, n_x=11, y_min=-1.0, y_max=1.0, n_y=11, labels=None):
    model = model.cuda() if gpu else model.cpu()
    ...
    for ratio in ratio_grid.reshape([-1, 2]):
        ...
        print("Grid: ", ratio, end=", ")
        # model, n_ff, dataset, train_mask, labels,
        *metrics, cal_diag = tests.test_rgnn(model=model, n_ff=n_ff, dataset=dataset, train_mask=train_mask,
                                        cutoffs=cutoffs, bins=bins, verbose=verbose, gpu=gpu, labels=labels)
        l1, l2 = norm.l1(model, gpu).item(), norm.l2(model, gpu).item()
        metrics_grid[tuple(ratio)] = (l1, l2, *metrics)
